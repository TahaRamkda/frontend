...
import { Container, Col, Card, CardBody, CardTitle, CardText, Button } from 'reactstrap';
  ...
  return {
    templates: [
      // Template 1: Starting template
      {
        ...
        headerParamCount: 0,
        bodyText: "Get 30% off your next meal, {{customername}}! Choose your favorite offer below.",
        bodyParamCount: 1,
        footerText: "Limited time offer",
        buttons: [
          { buttonId: 1, buttonText: "Chicken", buttonValue: "", buttonType: 1, sequence: 0, actionId: 2, actionType: 1 }, // Leads to template id: 2
          { buttonId: 2, buttonText: "Beef", buttonValue: "", buttonType: 1, sequence: 1, actionId: 3, actionType: 1 }, // Leads to template id: 3
          { buttonId: 3, buttonText: "Unsubscribe", buttonValue: "", buttonType: 1, sequence: 2, actionId: 0, actionType: 3 } // Unsubscribe action
        ],
        parameters: [
          { paramId: 1, paramName: "{{customername}}", paramType: 1, paramDefaultValue: "Alex", sequence: 0 }
        ]
      },
      // Template 2: Chicken offers
      {
        ...
        buttons: [
          { buttonId: 4, buttonText: "Mega Chicken Meal", buttonValue: "", buttonType: 1, sequence: 0, actionId: 4, actionType: 1 }, // Leads to template id: 4
          { buttonId: 5, buttonText: "Whopper Meal", buttonValue: "", buttonType: 1, sequence: 1, actionId: 5, actionType: 1 }, // Leads to template id: 5
          { buttonId: 6, buttonText: "Visit Website", buttonValue: "https://burgerking.com", buttonType: 3, sequence: 2, actionId: 0, actionType: 5 } // URL action (mapped to CHAT)
        ]
      },
      // Template 3: Beef offers
      {
        ...
        buttons: [
          { buttonId: 7, buttonText: "Mega Beef Meal", buttonValue: "", buttonType: 1, sequence: 0, actionId: 4, actionType: 1 }, // Leads to template id: 4
          { buttonId: 8, buttonText: "Ramadan Offer", buttonValue: "", buttonType: 1, sequence: 1, actionId: 6, actionType: 1 }, // Leads to template id: 6
          { buttonId: 9, buttonText: "Call Support", buttonValue: "+1234567890", buttonType: 2, sequence: 2, actionId: 0, actionType: 5 } // Call action (mapped to CHAT)
        ]
      },
      // Template 4: Meal details (used for Mega Chicken Meal and Mega Beef Meal)
      {
        ...
        buttons: [
          { buttonId: 10, buttonText: "Order Now", buttonValue: "", buttonType: 1, sequence: 0, actionId: 0, actionType: 6 }, // Order action
          { buttonId: 11, buttonText: "Chat Support", buttonValue: "", buttonType: 1, sequence: 1, actionId: 0, actionType: 5 } // Chat action
        ],
        parameters: [
          { paramId: 2, paramName: "{{mealname}}", paramType: 1, paramDefaultValue: "Meal", sequence: 0 },
          { paramId: 3, paramName: "{{price}}", paramType: 2, paramDefaultValue: "$10", sequence: 1 }
        ]
      },
      // Template 5: Meal details (used for Whopper Meal)
      {
        ...
        footerText: "Order now and save",
        buttons: [
          { buttonId: 12, buttonText: "Order Now", buttonValue: "", buttonType: 1, sequence: 0, actionId: 0, actionType: 6 },
          { buttonId: 13, buttonText: "Chat Support", buttonValue: "", buttonType: 1, sequence: 1, actionId: 0, actionType: 5 }
        ],
        parameters: [
          { paramId: 2, paramName: "{{mealname}}", paramType: 1, paramDefaultValue: "Meal", sequence: 0 },
          { paramId: 3, paramName: "{{price}}", paramType: 2, paramDefaultValue: "$10", sequence: 1 }
      ...
      // Template 6: Meal details (used for Ramadan Offer)
      {
        ...
        footerText: "Order now and save",
        buttons: [
          { buttonId: 14, buttonText: "Order Now", buttonValue: "", buttonType: 1, sequence: 0, actionId: 0, actionType: 6 },
          { buttonId: 15, buttonText: "Chat Support", buttonValue: "", buttonType: 1, sequence: 1, actionId: 0, actionType: 5 }
        ],
        parameters: [
          { paramId: 2, paramName: "{{mealname}}", paramType: 1, paramDefaultValue: "Meal", sequence: 0 },
          { paramId: 3, paramName: "{{price}}", paramType: 2, paramDefaultValue: "$10", sequence: 1 }
...
const renderBox = (id, title, content, buttons = [], actionDetails = {}) => (
  <Col xs="auto" key={id}>
    <Card id={id} className="mb-3 position-relative" style={{ width: '300px' }}>
      <CardBody>
        ...
        {buttons.map((button) => (
          <div 
            ...
            className="mb-2" 
            data-action={dropdownOptions.find(opt => opt.value === button.actionType)?.label.toLowerCase()}
            data-target={button.actionType === 1 && button.buttonType === 1 ? `template_${button.actionId}` : `action_${button.buttonId}`}
          >
        ...
        {!buttons.length && actionDetails.actionType !== undefined && (
          <CardText className="text-muted">
            {actionDetails.actionType === 0 ? 'No Action' :
             dropdownOptions.find(opt => opt.value === actionDetails.actionType)?.label || 'Unknown Action'}
          </CardText>
        )}
      </CardBody>
    </Card>
...
const collectNodesByLevel = (template, templateMap, level = 0, levels = {}, buttonMap = {}, visited = new Set()) => {
  ...
  template.buttons?.forEach((button) => {
    ...
    if (button.buttonType === 1) {
      ...
      actionTypeLabel = dropdownOptions.find(opt => opt.value === button.actionType)?.label.toLowerCase();
      if (button.actionType === 1) {
        ...
        if (nextTemplate) {
          ...
          // }
          collectNodesByLevel(nextTemplate, templateMap, level + 1, levels, buttonMap, visited);
        }
      } else {
        // Other action types (BLOCK, CHAT, ORDER, etc.)
        targetId = `action_${button.buttonId}`;
        const actionTitle = actionTypeLabel === "chat" ? "Chat With Agent" :
                           actionTypeLabel === "order" ? "Order" :
                           actionTypeLabel === "close chat" ? "Close Chat" :
                           actionTypeLabel === "flows" ? "Flow Action" :
                           actionTypeLabel === "unsubscribe" ? "Unsubscribe User" :
                           actionTypeLabel === "block" ? "Block User" :
                           "Action";
        const actionContent = actionTypeLabel === "chat" ? "Start Chat" :
                             actionTypeLabel === "order" ? "Place Order" :
                             actionTypeLabel === "close chat" ? "End Chat" :
                             actionTypeLabel === "flows" ? "Start Flow" :
                             actionTypeLabel === "unsubscribe" ? "Unsubscribed" :
                             actionTypeLabel === "block" ? "Blocked" :
                             actionTypeLabel;
...
export default function FlowVisualization({ initialData }) {
    ...
    const calculateLines = () => {
      ...
      const drawLines = () => {
        Object.entries(buttonMap).forEach(([buttonId, targetId], index) => {
          ...
          if (buttonRect.width === 0 || buttonRect.height === 0 || targetRect.width === 0 || targetRect.height === 0) {
          ...
          switch (action) {
            case "template": strokeColor = '#007bff'; break;
            case "chat": strokeColor = '#ffc107'; break;
            case "order": strokeColor = '#6f42c1'; break;
            case "close chat": strokeColor = '#fd7e14'; break;
            case "flows": strokeColor = '#20c997'; break;
            case "block": case "unsubscribe": strokeColor = '#dc3545'; break;
            default: strokeColor = '#6c757d';
          }
          ...
          const controlPoint1X = startX + (isTargetLeft ? -controlPointOffsetX : controlPointOffsetX);
          const controlPoint1Y = startY + controlPointOffsetY;
          const controlPoint2X = endX + (isTargetLeft ? controlPointOffsetX : -controlPointOffsetX);
  ...
  const maxCardsPerLevel = Math.max(...Object.values(levels).map(level => level.length));
  ...
  return (
    <>
      ...
      >
        ...
        >
          <h2 className="text-center mb-5">WhatsApp Template Flow Visualization</h2>
          <div ref={svgContainerRef} style={{ position: 'relative' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: "center" }}>
              {Object.keys(levels).map((level) => (
                <div key={level} className="mb-5" style={{ position: 'relative', zIndex: 0 }}>
                  <div 
                    style={{ 
                      display: 'flex', 
                      flexWrap: 'nowrap', 
