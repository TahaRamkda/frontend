...
import { Card, Col, Input, InputGroup, InputGroupText, Nav, NavItem, TabContent, TabPane, Container, Row, Button } from 'reactstrap';
...
const ChatPage = () => {
  ...
  const { conversationMessage, loading: messageLoading, error: messageError } = useSelector((state) => state.conversations);
  ...
  const HandleSendMessage = async () => {
    ...
    try {
      ...
      if (matchingConversationIndex !== -1) {
        ...
        console.warn("No matching conversation found for Activechat:", Activechat);
      }
    } catch (error) {
  ...
  useEffect(() => {
    ...
    const connection = new signalR.HubConnectionBuilder()
      .withUrl(`https://whatsappapi.consulttechies.com/conversation?AgentId=${UserId}`, {
    ...
    connection.on('MessageReceived', (message) => {
      ...
      if (message.id === activeChatRef.current) {
        ...
        if (matchingConversationIndex !== -1) {
          ...
          console.warn("No matching conversation found for message.id:", message.id);
        }
      } else {
    ...
    connection.on('ConversationAssigned', (notification) => {
      ...
      if (matchingConversationIndex !== -1) {
        ...
        console.warn("No matching conversation found for message.id:", notification.id);
        const newNotification = { ...notification, unreadCount: 1 };
        setAgentConversaton((prevMessages) => [newNotification, ...prevMessages]);
      }
  ...
  const startRecord = (stream) => {
    ...
    const recorder = new MediaRecorder(stream, { mimeType: isOggSupported ? mimeType : 'audio/webm' });
    ...
    recorder.onstop = () => {
      ...
      const audioBlob = new Blob(chunks, { type: isOggSupported ? mimeType : 'audio/mp3' });
        ...
        <Row className="g-0 h-100">
          <Col xxl="3" xl="4" md="5" className="box-col-5 p-0" style={{ height: '100vh' }}>
            <Card className="left-sidebar-wrapper h-100">
              ...
              <Nav tabs className="border-tab bg-white border-b border-gray-200 shadow-sm">
                ...
                <TabPane>
                  <ul className="divide-y divide-gray-200 chats-user overflow-y-auto">
                    ...
                    {AgentConversaton?.map((conversation) => (
                      <li
                        key={conversation.id}
                        className={`flex justify-between cursor-pointer px-4 py-0.5 rounded-lg transition-all duration-200 ease-in-out ${Activechat === conversation.id ? 'bg-gray-200' : 'hover:bg-gray-100'
                          }`}
                        onClick={() => {
          ...
          <Col xxl="9" xl="8" md="7" className="box-col-7 p-0" style={{ height: '100vh' }}>
              ...
              <div className="right-sidebar-chat p-4 w-full height-chat-box overflow-y-auto chat-background h-100">
                <div className="msger flex flex-col h-full">
                  ...
                  <div className="msger-inputs px-4 py-3 flex items-center">
                    <Button onClick={openFileManager} className="text-xl text-gray-500 hover:text-gray-700 mr-2">
                    ...
                    >
                      <i className={`fa fa-${isRecording ? 'stop' : 'microphone'}`} aria-hidden="true"></i>
                    </button>
      ...
      {Errordisconect && (
        <div className="fixed top-0 left-0 w-full h-full flex items-center justify-center bg-gray-900 bg-opacity-50 z-50">
          <div className="bg-red-500 p-8 rounded-lg shadow-md max-w-md w-full text-center">
            ...
            <h2 className="text-2xl font-semibold text-white mb-4">Disconnected</h2>
            <p className="text-white mb-6">You have been disconnected from the server.</p>
            <button
