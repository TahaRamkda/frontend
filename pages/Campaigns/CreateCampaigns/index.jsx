...
import { createCampaign, clearCampaignCreeateState} from "@/slices/CampaignSlice";
...
const CampaignCreate = () => {
  ...
  const { loading:campaingCreating, error :campaignerror} = useSelector((state) => state.templates);
  ...
  useEffect(() => {
    if (selectedTemplateId) { // Run only if a template is selected
      setLoading(true);
    ...
    try {
      ...
      if (response.success) {
        ...
        showSweetAlert({
          ...
          text: response.message || "Campaign has been reated successfully",
          icon: "success",
        });
